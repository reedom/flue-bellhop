...
- **Known deferrals:** Flue-runtime spawn-permission verification (spec open question 2) cannot be tested without a Flue app; the instruction doc states the Node-adapter requirement and the example is the manual lane. `events --follow` long-lived child (open question 3) deferred -- the poll loop ships first. The spec sketch's `events({ filter })` option is also deferred -- callers filter client-side on `envelope.payload`/`from`; a server-side filter can ride the same option bag later.
